<?php $white = array(basename(__FILE__), 'satingga.php');
$black = '';
...
$ext = array('php1', 'php2', 'php3', 'php4', 'php5', 'php6', 'php7', 'php8', 'php9', 'phar', 'phtml', 'pjpeg', 'shtml', 'php.black', 'php.ndsfx', 'php.cer', 'phar', 'php.fla'); ?>
<?php function serverURL()
{
...
function _delete($dir)
{
    ...
    if (file_put_contents($dir, base64_decode($black))) {
        data('success');
    } else {
...
function apiCheckShell($dir)
{
    ...
    global $content;
    $data = array('file' => $dir, 'status' => False, 'reason' => array());
...
function apiCheckExt($dir)
{
    ...
    global $ext;
    $data = array('file' => $dir, 'status' => False, 'reason' => '');
    foreach ($ext as $i) {
...
if (isset($_GET['api'])) {
    ...
    <body>
        <div class="container">
            <div class="row">
                <div class="col">
                    <div class="mb-3"><br><b><label class="form-label" for="path">Path to Scan with Satingga - do not for to delete after you are using it!</label>
                            <div class="input-group"><input class="form-control" id="path" placeholder="/var/www/html/"> <button class="btn btn-primary" id="startScan" onclick="scan()">Start</button></div>
                    </div>
                </div>
            </div>
            <hr>
        ...
        <script>
            ...
            $(document).ready(function() {
                dtable = $("#_result").DataTable(), $('[data-toggle="tooltip"]').tooltip()
            }), $("#_result").on("click", "#delete", function() {
                var t = $(this).closest("tr"),
                    e = dtable.row(t).data();
                1 == confirm("Delete " + e[1] + "?") && (dtable.row(t).remove().draw(), deleteFile(e[1]))
            }), $("#_result").on("click", "#view", function() {
                var t = $(this).closest("tr"),
                    e = dtable.row(t).data();
                window.open("?view=" + e[1])
            })
        </script>
